...
exports.STRUCTURED_LOG = async (req, res) => {
  ...
  const worker = require('@wardpeet/gatsby-plugin-structured-logging-worker/gatsby-worker');
...
exports.IMAGE_PROCESSING = async (req, res) => {
  const { id, inputPaths, outputDir, args } = req.body;
  const worker = require('@wardpeet/gatsby-plugin-sharp-worker/gatsby-worker');
...
exports.DOWNLOAD_FILE = async (req, res) => {
  const { id, inputPaths, outputDir, args } = req.body;
  const worker = require('@wardpeet/gatsby-plugin-remote-file/gatsby-worker');
...
exports.GENERATE_SVG = async (req, res) => {
  const { id, inputPaths, outputDir, args } = req.body;
  const worker = require('@wardpeet/gatsby-plugin-sqip-worker/gatsby-worker');
