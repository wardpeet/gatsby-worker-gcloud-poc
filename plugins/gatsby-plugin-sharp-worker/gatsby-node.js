...
const createJob = async (type, { inputPaths, outputDir, args }) => {
  ...
  if (process.send) {
    ...
    try {
      const result = await require('./gatsby-worker')[type]({ inputPaths, outputDir, args });
      return result;
    } catch (err) {
      reporter.panic(err);
    }
...
const promises = [];
exports.onPreInit = (actions, { inputPaths, ...args }) => {
  reporter = actions.reporter;
  promises.push(createJob('IMAGE_PROCESSING', {
    inputPaths,
